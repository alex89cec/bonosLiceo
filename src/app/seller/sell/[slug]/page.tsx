...
import FlyerPresentation from "@/components/FlyerPresentation";
  ...
  useEffect(() => {
    async function init() {
      ...
      const { data: campaign } = await supabase
        .from("campaigns")
        .select(
          "id, name, slug, ticket_price, installments_enabled, installments_count, description, flyer_url",
        )
        .eq("slug", slug)
        .single();
      ...
      <div>
        ...
        <div className="mb-4">
          <div className="flex items-start justify-between gap-3">
            ...
            {(campaignInfo?.flyer_url || campaignInfo?.description) && (
              <button
                type="button"
                onClick={() => setShowFlyer(true)}
                className="flex-shrink-0 rounded-xl bg-navy-700 px-3 py-2 text-xs font-semibold text-white transition hover:bg-navy-800"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="mb-0.5 inline h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* Flyer presentation overlay */}
        {showFlyer && campaignInfo && (
          <FlyerPresentation
            campaignName={campaignInfo.name}
            flyerUrl={campaignInfo.flyer_url}
            description={campaignInfo.description}
            ticketPrice={campaignInfo.ticket_price}
